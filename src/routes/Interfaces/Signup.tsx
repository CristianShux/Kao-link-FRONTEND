...
export const Signup = () => {
    ...
    const fetchLocalidadesPorProvincia = async () => {
      ...
      try {
        const response = await fetch(
          `https://kao-link-backend-gestion.onrender.com/api/localidades-filtrado/?codigo_provincia=${provinciaSeleccionada.codigo_provincia}`
        );
  ...
  useEffect(() => {
    const fetchPartidosPorProvincia = async () => {
      ...
      try {
        const response = await fetch(
          `https://kao-link-backend-gestion.onrender.com/api/partidos-filtrado/?codigo_provincia=${provinciaSeleccionada.codigo_provincia}`
        );
        const data = await response.json();
    ...
    const fetchProvincias = async () => {
      try {
        const response = await fetch('https://kao-link-backend-gestion.onrender.com/api/provincias');
        const data = await response.json();
        setProvincias(data);
...
try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // --- ¡¡AGREGÁ ESTO!! ---
    console.log("Preparando para crear usuario con:", {
        id_empleado: id_empleado,
        id_rol_a_enviar: credenciales.rol,
