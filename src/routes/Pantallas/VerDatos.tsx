...
export const VerDatos = () => {
  ...
  const manejarVerCV = async () => {
    ...
    const url = `https://kao-link-backend-gestion.onrender.com/api/documentos/CV/${usuario.id_empleado}`;
    try {
  ...
  const manejarVerTitulo = async () => {
    if (!usuario?.id_empleado) return;
    const url = `https://kao-link-backend-gestion.onrender.com/api/documentos/Título/${usuario.id_empleado}`;
    try {
      const response = await fetch(url);
