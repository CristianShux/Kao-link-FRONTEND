...
export const Asistencias = () => {
  ...
  const [asistenciasPorMes, setAsistenciasPorMes] = useState<
    Record<string, RegistroAsistencia[]>
  >({});
  ...
  useEffect(() => {
    const fetchAsistencias = async () => {
      try {
        ...
        const datosCrudos = await registroAsistenciasPorId(
          JSON.stringify(usuario?.id_empleado)
        );

        console.log("Datos crudos de asistencias:", datosCrudos);

        const asistenciasAdaptadas: RegistroAsistencia[] = datosCrudos.map(
          (registro: any[]) => {
            const [fecha, dia, horaEntrada, horaSalida, , horasExtras, estado] =
              registro;
            return {
              fecha: fecha || "---",
              dia: dia || "---",
        ...
        setMesSeleccionado(
          agrupadas[mesActual] ? mesActual : Object.keys(agrupadas)[0]
        );
      } catch (error) {
  ...
  return (
    <div className="layout">
      <div className="cont-asistencias">
        ...
        <h3>
          En esta sección podrás consultar tu historial de asistencias
          correspondiente al mes actual. Cada fila representa un día del mes,
          indicando tu hora de entrada, salida, estado de asistencia y si se
          registraron horas extras.
        </h3>

        <ul className="leyenda-abreviaciones">
          <li>
            <strong>HE</strong> → Hora de entrada
          </li>
          <li>
            <strong>HS</strong> → Hora de salida
          </li>
          <li>
            <strong>HT</strong> → Horas trabajadas
          </li>
          <li>
            <strong>HEX</strong> → Horas Extras
          </li>
        </ul>
        ...
        <div className="asistencias" style={{ position: "relative" }}>
          ...
          <table style={{ filter: cargando ? "blur(2px)" : "none" }}>
            ...
            <tbody>
              {(asistenciasPorMes[mesSeleccionado] || []).map(
                (asistencia, index) => {
                  const {
                    fecha,
                    dia,
